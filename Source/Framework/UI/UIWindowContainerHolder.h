...
public:
	UIWindowContainerHolder(std::unique_ptr<T> con, bool show = false)
		:
		UIWindow(show),
		m_pCon(std::move(con))
	{
	...
	UIWindowContainerHolder(bool show = false)
		:
		UIWindowContainerHolder(std::unique_ptr<T>(new T()),show)
	{}
