...
public:
	...
	{
		m_uiObjects.add(m_colorPicker.getRef(), m_numR.getRef(), m_numG.getRef(), m_numB.getRef(),
			m_lblR.getRef(), m_lblG.getRef(), m_lblB.getRef());
...
private:
	...
	refable<UIColorPicker> m_colorPicker;

	refable<UINumUpDownInt> m_numR;
	refable<UINumUpDownInt> m_numG;
	refable<UINumUpDownInt> m_numB;

	refable<UILabel> m_lblR;
	refable<UILabel> m_lblG;
	refable<UILabel> m_lblB;
